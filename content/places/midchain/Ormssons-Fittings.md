---
publish: true
aliases:
  - Ormsson's
title: Ormsson's Fittings
created: 2026-05-15
modified: 2026-05-15
tags:
  - building
---

# Ormsson's Fittings

## Overview

Ormsson's Fittings is on the first island, a short walk from the primary dry-dock cranes — close enough that the dry-dock workers account for half its foot traffic. [[Kalowe]]'s ships don't just need hull work; they need the fittings that go with it, and Kettil Ormsson has spent twenty years supplying them: anchor chain, cleats, deck bolts, caulking irons, and the kind of hardware that disappears into a refit and is never thought about again until it fails. He also keeps a small weapons selection that he does not put on a board outside, and sells it to anyone who does not explain themselves in too much detail.

## At a Glance

| Field | Detail |
|---|---|
| Type | Ironworks and hardware shop |
| Location | [[Kalowe]], first island, dry-dock quarter |
| Operated By | Kettil Ormsson |
| Public Access | Open during daylight hours |
| Known For | Dry-dock hardware and a back shelf nobody talks about |

## Description

Stone walls, a forge in the rear that Ormsson banks but rarely lights himself — most of the ironwork he sells was cast elsewhere and shipped in, though he does custom work for the dry dock on short notice. The front of the shop smells of machine grease and rust. Iron parts are sorted into open-topped crates along the walls, each with a chalk mark for the price. The counter is a workbench with a vise attached. A curtain at the back divides the retail area from the forge and the rear shelving, which is where the weapons live.

## Function

Hardware for ship repair and dry-dock work; general iron tools; a quiet arms trade for crews that need to resupply without involving a colonial factor.

## People & Operations

**Kettil Ormsson** — proprietor, late forties, broad-shouldered, from one of the northern island chains originally though he has been in the Midchain long enough to have lost most of the accent. He does not chat. He reads a customer's needs fast and gives a price with minimal elaboration. If someone asks about the back shelf without preamble, he tells them what's there. If they explain why they need it, he loses interest.

_Roleplay concept: a hardware man who found a free port because colonial smiths ask too many questions._

**Sample lines:**

- _"Chain's in the third crate. Per foot."_
- _"Bolt pattern for a Merrik-class cutter? Yeah, I've got that."_
- _"Curtain's open. What do you need?"_

No staff. He works the shop alone and closes when the dry dock does.

## Stock / Services

**General Hardware**

| Item | Cost | Notes |
|---|---|---|
| Iron spike (10) | 1 gp | Deck and hull fastening |
| Iron chain, per foot | 5 sp | Standard; heavier gauge available at +2 sp/ft |
| Anchor chain (10 ft section) | 6 gp | Drop-forged; rated for small vessels |
| Cleat, iron | 8 sp | Deck fittings; common sizes in stock |
| Padlock | 5 gp | Key included; re-key at 1 gp |
| Crowbar | 2 gp | — |
| Hammer | 1 gp | General; also caulking mallet at 1 gp 2 sp |
| Caulking iron | 2 gp | Dry-dock grade |
| Iron pot | 2 gp | Cook's or tar pot |
| Grappling hook | 2 gp | Four-prong; does not sell the line |
| Piton (10) | 5 sp | — |
| Block, iron-strapped | 1 gp 5 sp | Rigging pulley; oak shell |
| Lantern, iron-cased | 4 gp | Sturdier than the chandlery's; dimmer |
| Custom bolt pattern work | 5 gp + | Quoted per job; 1–2 day turnaround |

**Arms (back shelf — not advertised)**

| Item | Cost | Notes |
|---|---|---|
| Dagger | 2 gp | — |
| Handaxe | 5 gp | — |
| Spear | 1 gp | — |
| Shortsword | 10 gp | — |
| Rapier | 26 gp | Slightly above market; good quality |
| Crossbow, light | 25 gp | — |
| Crossbow bolts (20) | 1 gp | — |
| [[Flintlock-Pistol\|Flintlock pistol]] | 260 gp | Gray market; no provenance given |
| Pistol ammunition (20 shots, powder and ball) | 12 gp | Kept dry in waxed paper |
| [[Musket\|Musket]] | 510 gp | One in stock; ask |

## Access & Security

Front entrance from the dry-dock road. The curtain to the back is always partly open; Ormsson decides when to pull it fully aside. He locks the shop at sundown with a padlock that he changed the key on three years ago and has not since. The forge back door opens to a narrow yard; the yard gate is always latched.

## Current Hooks

- The _Takowan_'s boatswain placed a custom order two days ago — deck-bolt pattern specific to an enforcement-class cutter. Ormsson filled it without comment and noted it in no ledger.
- **Sawek Bounty** — The cracked keel plank propped against the outside wall came off a fishing boat the Sawek hit five days ago. Kettil is repairing it as a favour. He'll mention it if the party asks about local trouble: the harbour bosses are offering 300gp for the ridge-plate, the inter-island channel is closed at night, and one fisherman is dead. He doesn't editorialize. The bounty is real and the money is immediate. _(→ [[situations/Kalowe-Sawek-Bounty|The Sawek Bounty]])_

## See Also

- [[Kalowe]]
- [[Wibowos-Provisions|Wibowo's Provisions]]
- [[Taufa-Fifita-and-Sons|Taufa Fifita & Sons]]
